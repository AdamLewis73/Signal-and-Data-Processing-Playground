...
for i in range(M,N):
    # print('not here')
    # print(str(crossupperind[i]) + ' : ' + str(math.floor((crosslowerind[i]-crossupperind[i])/2)))
    # print(math.floor((crosslowerind[i]-crossupperind[i])/2))
...
for i in range(len(hor)):
    if ((slop[i] < 0 and slop[i - 1] > 0 and ynew3[i] > yml[0]) or (slop[i]< 0 and slop[i] > (-0.0001) and ynew3[i] > yml[0])):
        crosscount = crosscount + 1
        if count > 0:
            del testpeakpts[-1]
            del testpeakT[-1]
            del testpeakind[-1]
        testpeakpts.append(ynew3[i])
        testpeakT.append(hor[i])
        testpeakind.append((i))
        # if ((testpeakT[-1] - crossTupper[crosscount])<0.6):
        #     del testpeakpts[-1]
        #     del testpeakT[-1]
...
Q = 1
for i in range(P,Q):
    upsec.extend(emg_rec[crossupperind[i]:testpeakind[i]])
    upsecT.extend(hor[crossupperind[i]:testpeakind[i]])
...
ax0.plot(lowhalfT,lowhalfsec, color = 'red')

ax0.plot(testpeakT,testpeakpts, marker = "x",color = 'blue', linestyle = "None")
